...
const supabase = createClient(
  "https://rpmcwlkjvlwqdxnntwuu.supabase.co",
  "sb_publishable_uF0nI_18Whj_nW1Fy3VxkQ_lNbcRPYq"
);
import {
  Plus,
  X,
...
const nowTs = () => {
  ...
  return {
    ...
    timeStr: `${h % 12 || 12}:${String(m).padStart(2, "0")} ${
      h >= 12 ? "PM" : "AM"
    }`,
  };
};
...
const byWins = (ps) =>
  [...ps].sort((a, b) => b.wins - a.wins || a.losses - b.losses);
const medal = (r) =>
  r === 1
    ? { e: "🥇", c: "#FFD700" }
    : r === 2
...
const GridBg = () => (
  <div
    ...
    style={{
      backgroundImage:
        "linear-gradient(rgba(170,255,0,.028) 1px,transparent 1px),linear-gradient(90deg,rgba(170,255,0,.028) 1px,transparent 1px)",
  ...
  <div className="pointer-events-none fixed inset-0 z-0 overflow-hidden">
    <motion.div
      ...
      style={{
        ...
        borderRadius: "50%",
        background:
          "radial-gradient(circle,rgba(170,255,0,.07) 0%,transparent 70%)",
        top: -150,
        right: -120,
        filter: "blur(50px)",
      }}
    ...
    <motion.div
      ...
      style={{
        ...
        borderRadius: "50%",
        background:
          "radial-gradient(circle,rgba(170,255,0,.05) 0%,transparent 70%)",
        bottom: -120,
        left: -100,
        filter: "blur(60px)",
      }}
...
function AnimBar({ value, color }) {
  ...
  return (
    <div
      ...
      style={{ height: 18, background: "rgba(255,255,255,.06)" }}
    >
      <motion.div
  ...
  return (
    <motion.button
      ...
      style={{
        ...
        color: disabled ? "rgba(170,255,0,.28)" : "#000",
        boxShadow: disabled ? "none" : "0 6px 24px rgba(170,255,0,.3)",
      }}
    >
      {!disabled && (
        <div
...
function StandingsTable({ players, feed = [] }) {
  ...
  return (
    ...
    >
      <div
        className="grid px-3 py-2.5 gap-1"
        style={{
      ...
      {rows.map((p, i) => {
        const r = i + 1;
        const md = medal(r);
        const ep = enriched.find((e) => e.id === p.id) || {
          gamesWon: 0,
          gamesLost: 0,
        };
        const winPct =
          p.totalPlayed > 0 ? Math.round((p.wins / p.totalPlayed) * 100) : 0;
        const pctC = winPct >= 60 ? N : winPct >= 40 ? "#FFB830" : "#FF3355";
        return (
          <div
            key={p.id}
            className="grid px-3 py-3 items-center gap-1 hover:brightness-110 transition-all"
            style={{
              gridTemplateColumns: COL,
              borderBottom:
                i < rows.length - 1
                  ...
                  : "none",
              background: p.isMe ? "rgba(170,255,0,.04)" : "transparent",
            }}
          >
            <div
              style={{
                fontFamily: "'Bebas Neue',sans-serif",
                fontSize: md ? 16 : 20,
                color: md ? md.c : "rgba(255,255,255,.25)",
              }}
            >
              {md ? md.e : r}
            </div>
            <div className="flex items-center gap-2 min-w-0">
              <div
                ...
                style={{ background: pg(p), color: "#000" }}
              >
                {p.initials}
              </div>
              <div className="min-w-0">
                ...
                <div className="flex items-center gap-1 mt-0.5">
                  {p.trend === "up" && (
                    <TrendingUp size={8} style={{ color: N }} />
                  )}
                  {p.trend === "down" && (
                    <TrendingDown size={8} style={{ color: "#FF3355" }} />
                  )}
                  {p.trend === "flat" && (
                    <Minus
                      size={8}
            ...
            >
              <span style={{ color: N }}>{p.wins}</span>
              <span style={{ color: "rgba(255,255,255,.2)" }}>/</span>
              <span style={{ color: "#FF3355" }}>{p.losses}</span>
            </div>
            <div
              className="text-center"
              style={{
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3 px-4 pt-4 pb-3">
        <div
          className="flex items-center justify-center rounded-[11px] text-xl flex-shrink-0"
          style={{
            width: 38,
            height: 38,
      ...
      >
        ...
        >
          <Clock size={10} style={{ color: "rgba(255,255,255,.25)" }} />
          {m.timeStr}
        </div>
      </div>
    </motion.div>
...
function LogModal({ players, onClose, onSubmit, prefill = null }) {
  ...
  const upd = (i, side, v) =>
    setSets((p) => p.map((s, ix) => (ix === i ? { ...s, [side]: v } : s)));
  const submit = () => {
    setStep(2);
    setTimeout(() => {
      onSubmit({ winners, losers, sets, editId: prefill?.id });
      onClose();
    }, 1500);
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div>
            ...
            {step < 2 && (
              ...
              >
                {step === 0
                  ? "Select winner(s) and loser(s)"
                  : "Enter the score for each mini-game"}
              </p>
            )}
          </div>
          {step < 2 && (
          ...
          <div className="flex gap-1.5 px-5 pt-3 flex-shrink-0">
            {[0, 1].map((i) => (
              <div
                ...
                style={{ height: 2.5, background: "rgba(255,255,255,.07)" }}
              >
                <motion.div
        ...
        <div className="overflow-y-auto flex-1">
          <AnimatePresence mode="wait">
            ...
            {step === 0 && (
              ...
              >
                {[
                  { lbl: "WINNERS 🏆", side: "w", c: N },
                  { lbl: "LOSERS 💀", side: "l", c: "#FF3355" },
                ].map(({ lbl, side, c }, si) => (
                  <div key={lbl}>
                    <p
                      style={{
                    ...
                    {players.map((p) => {
                      const sel = (side === "w" ? winners : losers).includes(
                        p.id
                      );
                      const dim = (side === "w" ? losers : winners).includes(
                        p.id
                      );
                      return (
                        <button
                          key={p.id}
                          onClick={() => tog(p.id, side)}
                          disabled={dim}
                        ...
                        >
                          ...
                          <span
                            style={{
                              fontSize: 14,
                              fontWeight: 600,
                              color: sel ? "#fff" : "rgba(255,255,255,.65)",
                              fontFamily: "'DM Sans',sans-serif",
                            }}
                          >
                            {p.name}
                            {p.isMe && (
                              <span style={{ fontSize: 10, color: N }}>
                                {" "}
                                · YOU
                              </span>
                            )}
                          ...
                          {sel && (
                            <div
                              ...
                              style={{
                                background: side === "w" ? N : "#FF3355",
                              }}
                            >
                              <Check
                                size={11}
                                color={side === "w" ? "#000" : "#fff"}
                                strokeWidth={3}
                              />
                            </div>
                          )}
            ...
            {step === 1 && (
              ...
              >
                {/* Match summary pill */}
                <div
                  className="flex items-center rounded-[14px] px-4 py-3 mb-5"
                  style={{
                    background: "rgba(255,255,255,.04)",
                    border: "1px solid rgba(255,255,255,.08)",
                  }}
        ...
        {step < 2 && (
          <div
            className="px-5 pb-6 pt-4 flex-shrink-0"
            style={{
            ...
            {step === 0 && (
              ...
              >
                ...
                <span style={{ position: "relative" }}>
                  {canGo ? "Set Scores →" : "Select Players First"}
                </span>
              </button>
            )}
            {step === 1 && (
...
function HomeTab({ players, feed, onEditFeed }) {
  ...
  const mvp = useMemo(
    () =>
      players.length > 0
        ? [...players].sort((a, b) => b.wins - a.wins)[0]
        : { name: "No Players", wins: 0, losses: 0 },
    [players]
  );
  const injured = useMemo(
    () =>
      players.length > 0
        ? [...players].sort((a, b) => a.totalPlayed - b.totalPlayed)[0]
        : { name: "No Players", totalPlayed: 0 },
    [players]
  );
  const clutch = useMemo(
    () =>
      players.length > 0
        ? [...players].sort(
            (a, b) => (b.clutchWins || 0) - (a.clutchWins || 0)
          )[0]
        : { name: "No Players", clutchWins: 0 },
    [players]
  );
  ...
  return (
    <div className="px-5 pt-5 pb-2">
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        ...
        >
          <div
            ...
            style={{
              background:
                "linear-gradient(135deg,rgba(255,215,0,.05),transparent 60%)",
            }}
          />
        ...
        >
          <div
            ...
            style={{
              background:
                "linear-gradient(135deg,rgba(255,51,85,.05),transparent 60%)",
            }}
          />
          ...
          <div
            style={{
              fontSize: 9,
              fontWeight: 700,
              color: "rgba(255,51,85,.5)",
              fontFamily: "'DM Sans',sans-serif",
              marginTop: 2,
            }}
          >
            {injured.totalPlayed === 0 ? "NO MATCHES" : "MATCHES"}
          </div>
        </div>
      </div>
      ...
      {/* AI Ref */}
      <div
        className="flex items-center gap-4 rounded-[20px] p-4 mb-6 relative overflow-hidden cursor-pointer hover:brightness-110 transition-all"
        style={{
          background: "rgba(255,255,255,.03)",
      ...
      >
        <div
          ...
          style={{
            background:
              "linear-gradient(135deg,rgba(170,255,0,.04),transparent 60%)",
          }}
        />
        <div
          className="w-11 h-11 rounded-[13px] flex items-center justify-center text-xl flex-shrink-0 relative z-10"
          style={{
            background: `linear-gradient(135deg,${N},#7DC900)`,
            boxShadow: "0 4px 16px rgba(170,255,0,.3)",
          }}
        >
          🤖
        </div>
        ...
        <ChevronRight
          size={18}
          style={{ color: N, flexShrink: 0, position: "relative", zIndex: 10 }}
        />
      </div>

      <ST>⚡ League Feed</ST>
      {visible.map((m) => (
        <FeedCard key={m.id} m={m} onEdit={onEditFeed} players={players} />
      ))}
...
function StatsTab({ players, feed }) {
  ...
  function LbView() {
    ...
    const mP =
      players.length > 0
        ? [...players].sort((a, b) => b.totalPlayed - a.totalPlayed)[0]
        : FALLBACK;
    const mL =
      players.length > 0
        ? [...players].sort((a, b) => b.losses - a.losses)[0]
        : FALLBACK;
    const mGM =
      enriched.length > 0
        ? [...enriched].sort((a, b) => b.gamesWon - a.gamesWon)[0]
        : FALLBACK;
    const mGL =
      enriched.length > 0
        ? [...enriched].sort((a, b) => b.gamesLost - a.gamesLost)[0]
        : FALLBACK;
    const mClutch =
      players.length > 0
        ? [...players].sort(
    ...
    const mStreak =
      players.length > 0
        ...
        : FALLBACK;
    const rows = useMemo(() => byWins(players), []);
    const barC = (pc) =>
      pc >= 70
        ? `linear-gradient(90deg,${N},#7DC900)`
        : pc >= 50
        ? "linear-gradient(90deg,#FFB830,#E08A00)"
        : "linear-gradient(90deg,#FF3355,#C0143C)";
    const ALL_AWARDS = [
      {
        icon: "🏆",
        lbl: "THE WINNER",
        lc: "rgba(170,255,0,.7)",
        sc: N,
        bdr: "rgba(170,255,0,.2)",
        p: mW,
        bigNum: mW.wins,
        unit: "WINS",
        stat: `${pct(mW.wins, mW.losses)}% win rate`,
        hasTrend: true,
      },
      {
        icon: "⚡",
        lbl: "CLUTCH PLAYER",
        lc: "rgba(59,142,255,.7)",
        sc: "#3B8EFF",
        bdr: "rgba(59,142,255,.2)",
        p: mClutch,
        bigNum: mClutch.clutchWins || 0,
        unit: "CLUTCH WINS",
        stat: `Tight-score wins`,
        hasTrend: false,
      },
      {
        icon: "🔥",
        ...
        bdr: "rgba(170,255,0,.2)",
        p: mStreak,
        bigNum: mStreak.bestStreak || 0,
        unit: "WIN STREAK",
        stat: `Best consecutive wins`,
        hasTrend: false,
      },
      {
        icon: "⚙️",
        lbl: "THE GRINDER",
        lc: "rgba(255,184,48,.7)",
        sc: "#FFB830",
        bdr: "rgba(255,184,48,.2)",
        p: mP,
        bigNum: mP.totalPlayed,
        unit: "MATCHES",
      ...
      {
        icon: "💀",
        lbl: "PROFESSIONAL LOSER",
        lc: "rgba(255,51,85,.7)",
        sc: "#FF3355",
        bdr: "rgba(255,51,85,.2)",
        p: mL,
        bigNum: mL.losses,
        unit: "LOSSES",
        stat: `Most losses on record`,
        hasTrend: true,
      },
      {
        icon: "🎯",
        ...
        bigNum: mGM.gamesWon,
        unit: "MINI-GAMES",
        stat: `${mGM.gamesWon} mini-games won`,
        hasTrend: true,
      },
      {
        icon: "🩹",
        lbl: "THE VICTIM",
        lc: "rgba(170,85,255,.7)",
        sc: "#AA55FF",
        bdr: "rgba(170,85,255,.2)",
        p: mGL,
        bigNum: mGL.gamesLost,
        unit: "GAMES LOST",
        stat: `${mGL.gamesLost} mini-games lost`,
        hasTrend: true,
      },
    ];
    const partners = me.partners
      ? Object.entries(me.partners)
          .map(([id, p]) => ({
            pp: players.find((x) => x.id === Number(id)),
            pc: Math.round(p * 100),
          }))
          .filter((x) => x.pp)
          .sort((a, b) => b.pc - a.pc)
      ...
      <div>
          ...
          {ALL_AWARDS.map((c) => (
            ...
            >
              <div
                className="absolute inset-0 pointer-events-none"
                style={{
                  background: `linear-gradient(135deg,${c.sc}08,transparent 60%)`,
                }}
              />
              <div className="flex items-center justify-between gap-3 relative">
                <div className="flex items-start gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    {c.hasTrend && (
                      <Sparkline
                        data={c.p.mvTrend || [0, 0, 0, 0, 0, 0, 0]}
                        color={c.sc}
                        w={100}
                        h={20}
                      />
        ...
        >
          <div className="grid grid-cols-2 gap-4 mb-4">
            {[
              { v: me.gamesWon, l: "MINI-GAMES WON", c: N },
              { v: me.gamesLost, l: "MINI-GAMES LOST", c: "#FF3355" },
            ].map(({ v, l, c }) => (
          ...
          >
            <div
              style={{
                width: `${Math.round((me.gamesWon / totG) * 100)}%`,
            ...
            <div
              style={{
                flex: 1,
                background: "linear-gradient(90deg,#FF3355,#C0143C)",
              }}
            />
          </div>
          <div className="flex justify-between mt-2">
            ...
            >
              {Math.round((me.gamesWon / totG) * 100)}% won
            </span>
            <span
              style={{
                fontSize: 10,
            ...
            >
              {me.gamesWon + me.gamesLost} total
            </span>
            <span
              style={{
                fontSize: 10,
            ...
            >
              {Math.round((me.gamesLost / totG) * 100)}% lost
            </span>
          </div>
        </div>
      </div>
  ...
  function H2HView() {
    ...
    const h2h = useMemo(() => {
      ...
      (feed || []).forEach((m) => {
        ...
        const meW = wIds.includes(me.id),
          meL = lIds.includes(me.id);
        const rivW = wIds.includes(rival.id),
          rivL = lIds.includes(rival.id);
    ...
    const vrd = useMemo(() => {
      ...
      if (!h2h.l)
        return {
          icon: "👑",
          title: "UNTOUCHABLE",
          text: `${rival.name} has NEVER beaten you.`,
          bg: "rgba(170,255,0,.07)",
          bdr: "rgba(170,255,0,.25)",
          color: N,
          badge: "PERFECT RECORD",
        };
      if (!h2h.w)
        return {
          icon: "😰",
          title: "YOUR NIGHTMARE",
          text: `Zero wins vs ${rival.name}. Trauma.`,
          bg: "rgba(255,51,85,.07)",
          bdr: "rgba(255,51,85,.25)",
          color: "#FF3355",
          badge: "0 WINS",
        };
      if (r >= 0.8)
        return {
          icon: "💪",
          title: "DELIVERY BOY",
          text: `${Math.round(r * 100)}% win rate vs them.`,
          bg: "rgba(170,255,0,.07)",
          bdr: "rgba(170,255,0,.22)",
          color: N,
          badge: "DOMINANT",
        };
      if (r >= 0.6)
        return {
          icon: "📈",
          title: "SLIGHT EDGE",
          text: `You lead ${h2h.w}–${h2h.l}.`,
          bg: "rgba(170,255,0,.05)",
          bdr: "rgba(170,255,0,.15)",
          color: N,
          badge: "AHEAD",
        };
      if (r >= 0.45)
        return {
          icon: "⚔️",
          title: "DEAD HEAT",
          text: `${h2h.w}–${h2h.l}. Every match counts.`,
          bg: "rgba(255,184,48,.06)",
          bdr: "rgba(255,184,48,.22)",
          color: "#FFB830",
          badge: "EVEN",
        };
      if (r >= 0.3)
        return {
          icon: "📉",
          title: "LOSING GROUND",
          text: `${rival.name} leads ${h2h.l}–${h2h.w}.`,
          bg: "rgba(255,107,53,.06)",
          bdr: "rgba(255,107,53,.22)",
          color: "#FF6B35",
          badge: "BEHIND",
        };
      return {
        icon: "🚨",
        title: "NIGHTMARE MATCHUP",
        text: `${rival.name} dominates ${h2h.l}–${h2h.w}.`,
        bg: "rgba(255,51,85,.07)",
        bdr: "rgba(255,51,85,.25)",
        color: "#FF3355",
        badge: "LOSING BADLY",
        ...
        <div className="grid grid-cols-2 gap-2 mb-5">
          {rivals.map((p) => (
            <motion.button
              ...
              whileTap={{ scale: 0.96 }}
              onClick={() =>
                setRivalId((prev) => (prev === p.id ? null : p.id))
              }
            ...
            >
              <div
                className="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center font-black text-[11px]"
                style={{
                  background: pg(p),
                  color: "#000",
                  fontFamily: "'Bebas Neue',sans-serif",
        ...
        <AnimatePresence mode="wait">
          {rival ? (
            ...
            >
              ...
              {/* ── Score + progress bar ── */}
              <div
                className="rounded-b-[22px] px-5 py-4 mb-4"
                style={{
                  background: "rgba(255,255,255,.03)",
              ...
              >
                ...
                <div
                  ...
                  style={{ height: 8, background: "rgba(255,255,255,.08)" }}
                >
                  <motion.div
                    style={{
                      background: `linear-gradient(90deg,${N},#7DC900)`,
                      borderRadius: total === 0 ? "6px" : "6px 0 0 6px",
                    }}
                    initial={{ width: "0%" }}
                    animate={{ width: `${yp}%` }}
              ...
              {vrd && (
                <div
                  className="rounded-[18px] p-4 flex items-start gap-4 mb-4"
                  style={{ background: vrd.bg, border: `1px solid ${vrd.bdr}` }}
                >
                  ...
                  <div>
                    ...
                    {vrd.badge && (
                      <div
                        className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg mt-2"
                        style={{
                          background: `${vrd.color}18`,
                          border: `1px solid ${vrd.color}44`,
                          fontSize: 9,
                          fontWeight: 800,
                          letterSpacing: "1px",
                          color: vrd.color,
  ...
  return (
    <div className="px-5 pt-5 pb-2">
      <div className="flex gap-2 mb-5">
        {[
          { id: "lb", lbl: "Leaderboard" },
          { id: "h2h", lbl: "Head-to-Head" },
        ].map((t) => (
          <button
            key={t.id}
            onClick={() => setSub(t.id)}
            ...
            style={{
              background:
                sub === t.id ? "rgba(170,255,0,.1)" : "rgba(255,255,255,.04)",
              border:
                sub === t.id
                  ? "1px solid rgba(170,255,0,.35)"
                  : "1px solid rgba(255,255,255,.08)",
              color: sub === t.id ? N : "rgba(255,255,255,.4)",
              fontFamily: "'DM Sans',sans-serif",
              cursor: "pointer",
            }}
          >
...
function LeagueTab({
  ...
  const TOOLS = [
    {
      Icon: Settings,
      bg: `linear-gradient(135deg,${N},#7DC900)`,
      bdr: "rgba(170,255,0,.22)",
      lbl: "Edit Rules",
      sub: "Modify format, scoring, season",
      arr: N,
      fn: () => setEditing(true),
    },
    {
      Icon: UserPlus,
      bg: "linear-gradient(135deg,#3B8EFF,#1a6be0)",
      bdr: "rgba(59,142,255,.22)",
      lbl: "Add / Invite Players",
      sub: "Share invite link or add member",
      arr: "#3B8EFF",
      fn: onAddPlayer,
    },
    {
      Icon: UserMinus,
      bg: "linear-gradient(135deg,#FFB830,#E08A00)",
      bdr: "rgba(255,184,48,.22)",
      lbl: "Remove Players",
      sub: "Remove a member this season",
      arr: "#FFB830",
      fn: onRemovePlayer,
    },
    {
      Icon: RotateCcw,
      bg: "linear-gradient(135deg,#FF3355,#C0143C)",
      bdr: "rgba(255,51,85,.22)",
      lbl: "Reset Season",
      sub: "Clear all stats and start fresh",
      arr: "#FF3355",
      fn: () => setConfirm(true),
    },
  ];
  ...
  return (
    <div className="px-5 pt-5 pb-2">
      <ST>📸 Squad Photo</ST>
      <div
        className="rounded-[20px] mb-6 flex flex-col items-center justify-center gap-3 cursor-pointer hover:brightness-110 transition-all"
        style={{
          background: "rgba(255,255,255,.03)",
      ...
      >
        {editing ? (
            ...
            {[
              ...
              <div key={f.k}>
                ...
                <input
                  value={draft[f.k]}
                  onChange={(e) =>
                    setDraft((d) => ({ ...d, [f.k]: e.target.value }))
                  }
          ...
          [
            ...
            ["🏆", "Season", `Season ${rules.seasonYear}`],
          ].map(([ic, lb, vl]) => (
        ...
        {confirm && (
          ...
          >
            ...
            >
              ...
              >
                All wins, losses, and streaks will be cleared. Cannot be undone.
              </p>
              <div className="flex gap-3">
                <button
                ...
                <button
                  ...
                  className="flex-1 rounded-[16px] py-3.5 font-bold text-sm"
                  style={{
                    background: "linear-gradient(135deg,#FF3355,#C0143C)",
                    color: "#fff",
                    border: "none",
                    cursor: "pointer",
                    fontFamily: "'DM Sans',sans-serif",
...
function ProfileTab({ players, feed, user = null }) {
  ...
  const mv = (me.wins * 0.5 + meE.gamesWon * 0.1).toFixed(1);
  const myMatches = useMemo(
    () =>
      feed.filter(
        (m) =>
          (m.winnerIds || []).includes(me.id) ||
          (m.loserIds || []).includes(me.id)
      ),
    [feed, me.id]
  );
      ...
      [
        {
          ...
          v: Math.min(
            99,
            Math.round(50 + wr / 2 + (me.streak > 0 ? me.streak * 3 : 0))
          ),
          c: N,
        },
        {
          l: "Power",
          v: Math.min(99, Math.round(40 + meE.gamesWon * 0.8)),
          c: "#3B8EFF",
        },
        {
          l: "Reliability",
          v: Math.min(99, Math.round((me.totalPlayed / 14) * 85 + 10)),
          c: "#FFB830",
        },
        {
          l: "Stamina",
          v: Math.min(99, Math.round(45 + me.totalPlayed * 2.5)),
          c: "#AA55FF",
        },
  ...
  const mgChampion = useMemo(
    () => [...enriched].sort((a, b) => b.gamesWon - a.gamesWon)[0],
    [enriched]
  );
  const isMVP = mvpPlayer.id === me.id;
  const isMGChamp = mgChampion.id === me.id;
  const clutchLeader = useMemo(
    () =>
      [...players].sort((a, b) => (b.clutchWins || 0) - (a.clutchWins || 0))[0],
    [players]
  );
  const streakLeader = useMemo(
    () =>
      [...players].sort((a, b) => (b.bestStreak || 0) - (a.bestStreak || 0))[0],
    [players]
  );
  ...
  const badges = [
    {
      ...
      s: "5 in a row",
      earned: me.streak >= 5,
      bc: "rgba(170,255,0,.15)",
      bb: "rgba(170,255,0,.35)",
    },
    {
      i: "👑",
      l: "Comeback King",
      s: "Trailed, won",
      earned: true,
      bc: "rgba(255,184,48,.12)",
      bb: "rgba(255,184,48,.35)",
    },
    {
      i: "⚡",
      l: "The Killer",
      s: "Bagel victory",
      earned: true,
      bc: "rgba(255,51,85,.12)",
      bb: "rgba(255,51,85,.35)",
    },
    {
      i: "📈",
      l: "Serial Winner",
      s: "+7 wins",
      earned: me.wins >= 7,
      bc: "rgba(59,142,255,.12)",
      bb: "rgba(59,142,255,.35)",
    },
    {
      i: "🎯",
      l: "Sniper",
      s: "High accuracy",
      earned: wr >= 50,
      bc: "rgba(170,85,255,.12)",
      bb: "rgba(170,85,255,.35)",
    },
    {
      i: "🛡️",
      l: "Iron Wall",
      s: "Lost <20 games",
      earned: meE.gamesLost < 20,
      bc: "rgba(0,229,204,.12)",
      bb: "rgba(0,229,204,.35)",
    },
  ];
  ...
  return (
    <div className="pb-4">
      <div
        ...
        style={{
          background:
            "linear-gradient(180deg,rgba(170,255,0,.06) 0%,transparent 100%)",
          borderBottom: "1px solid rgba(255,255,255,.07)",
          paddingBottom: 24,
        }}
      >
        ...
        >
          <div>
            ...
            <div
              style={{
                fontSize: 14,
                fontWeight: 800,
                color: ready ? N : "rgba(255,255,255,.5)",
                fontFamily: "'DM Sans',sans-serif",
              }}
            >
              {ready ? "✅ Ready to Play" : "😴 Resting"}
            </div>
          </div>
          <motion.button
          ...
          >
            <motion.div
              ...
              animate={{
                left: ready ? "calc(100% - 26px)" : "3px",
                background: ready ? N : "rgba(255,255,255,.35)",
              }}
            ...
            {ready && (
              <motion.div
                ...
                animate={{
                  boxShadow: [
                    "0 0 0 0 rgba(170,255,0,.5)",
                    "0 0 0 8px rgba(170,255,0,0)",
                  ],
                }}
            ...
            <motion.div
              animate={
                ready
                  ? {
                      boxShadow: [
                        "0 0 0 0 rgba(170,255,0,.4)",
                        "0 0 0 16px rgba(170,255,0,0)",
                        "0 0 0 0 rgba(170,255,0,0)",
                      ],
                    }
                  : {}
              }
        ...
        <motion.div {...sg(2)} className="text-center px-5 mb-4">
          ...
          >
            {user?.user_metadata?.full_name || me.name}
          </div>
          {user?.email && (
            <div
              style={{
          ...
          <div className="flex justify-center gap-5">
            {[
              { v: `${me.wins}W`, l: "WINS", c: N },
              { v: `${me.losses}L`, l: "LOSSES", c: "#FF3355" },
              { v: `${wr}%`, l: "WIN RATE", c: "#FFB830" },
            ].map((s) => (
        ...
        <motion.div {...sg(3)} className="mx-5">
          {editQ ? (
            ...
            <div
              className="flex items-center gap-2 rounded-[14px] px-4 py-3 cursor-pointer"
              style={{
                background: "rgba(255,255,255,.04)",
                border: "1px solid rgba(255,255,255,.08)",
              }}
            ...
            {[
              {
                icon: "🥇",
                t: "MVP OF THE DAY",
                holder: mvpPlayer.name,
                stat: `${mvpPlayer.wins} wins`,
                bg: "rgba(255,215,0,.06)",
                bdr: "rgba(255,215,0,.2)",
                tc: "rgba(255,215,0,.8)",
                mine: isMVP,
                mc: N,
              },
              {
                icon: "🎮",
                t: "MINI-GAME CHAMP",
                holder: mgChampion.name,
                stat: `${mgChampion.gamesWon} MG`,
                bg: "rgba(59,142,255,.06)",
                bdr: "rgba(59,142,255,.2)",
                tc: "rgba(59,142,255,.8)",
                mine: isMGChamp,
                mc: "#3B8EFF",
              },
              {
                icon: "⚡",
                t: "CLUTCH PLAYER",
                holder: clutchLeader.name,
                stat: `${clutchLeader.clutchWins || 0} clutch wins`,
                bg: "rgba(59,142,255,.06)",
                bdr: "rgba(59,142,255,.2)",
                tc: "rgba(59,142,255,.8)",
                mine: isClutchLeader,
              ...
              },
              {
                icon: "🔥",
                t: "SUPER STREAK",
                holder: streakLeader.name,
                stat: `${streakLeader.bestStreak || 0} win streak`,
                bg: "rgba(170,255,0,.06)",
                bdr: "rgba(170,255,0,.2)",
                tc: "rgba(170,255,0,.8)",
                mine: isStreakLeader,
                mc: N,
        ...
        <motion.div {...sg(6)}>
          ...
          <div className="grid grid-cols-3 gap-2.5 mb-6">
            {badges.map((b, i) => (
              <motion.div
                key={i}
                whileHover={b.earned ? { scale: 1.04, y: -2 } : {}}
                whileTap={b.earned ? { scale: 0.97 } : {}}
                className="rounded-[18px] py-4 px-3 text-center"
                style={{
                  background: b.bc,
                  border: `1px solid ${b.bb}`,
              ...
              >
                ...
                <div
                  style={{
                    ...
                    fontSize: 11,
                    letterSpacing: ".5px",
                    color: b.earned ? "#fff" : "rgba(255,255,255,.4)",
                    marginBottom: 2,
                  }}
                >
                  {b.l}
          ...
          {myMatches.length === 0 ? (
            ...
            <div className="flex flex-col gap-3 pb-4">
              {myMatches.map((m, i) => {
                ...
                const totalMG = (m.sets || []).reduce((a, s) => {
                  const { w, l } = parseMG(s);
                  return a + w + l;
                }, 0);
                return (
                  <motion.div
                    ...
                    style={{
                      background: "rgba(255,255,255,.03)",
                      border: `1px solid ${
                        isWin ? "rgba(170,255,0,.2)" : "rgba(255,51,85,.2)"
                      }`,
                    }}
                  >
                    <div className="flex items-center gap-3 px-4 py-3.5">
                      <div
                        className="flex items-center justify-center rounded-[10px] font-black text-xs flex-shrink-0"
                        style={{
                          width: 44,
                          height: 36,
                          background: isWin
                        ...
                        <div className="flex items-center gap-1 mb-1 flex-wrap">
                          <span
                            style={{
                              fontSize: 13,
                              fontWeight: 700,
                              color: isWin ? N : "rgba(255,255,255,.7)",
                              fontFamily: "'DM Sans',sans-serif",
                            }}
                          >
                            {m.winner}
      ...
      <div className="flex">
        {tabs.map(({ id, Icon, lbl }) => (
          ...
          >
            <Icon
              size={20}
              style={{ color: active === id ? N : "rgba(255,255,255,.28)" }}
            ...
            <span
              style={{
                ...
                fontWeight: 700,
                letterSpacing: ".5px",
                color: active === id ? N : "rgba(255,255,255,.28)",
                fontFamily: "'DM Sans',sans-serif",
              }}
            >
              {lbl.toUpperCase()}
...
/* ── ROOT ── */
function LeagueItApp({
  initialPlayers = INIT_PLAYERS,
  initialFeed = INIT_FEED,
  leagueId = null,
    ...
    ({ winners, losers, sets, editId }) => {
      ...
      players.forEach((p) => {
        ...
        updatedStats[p.id] = {
          ...
          streak: ns,
          clutchWins:
            isW && isClutch ? (p.clutchWins || 0) + 1 : p.clutchWins || 0,
      ...
      setPlayers((prev) =>
        prev.map((p) =>
          updatedStats[p.id] ? { ...p, ...updatedStats[p.id] } : p
        )
      );

      const wPlayers = winners
        .map((id) => players.find((p) => p.id === id))
        .filter(Boolean);
      const lPlayers = losers
        .map((id) => players.find((p) => p.id === id))
        .filter(Boolean);
      ...
      setFeed((prev) =>
        editId
          ? prev.map((m) => (m.id === editId ? entry : m))
          : [entry, ...prev]
      );
      ...
      if (leagueId) {
        ...
        Object.entries(updatedStats).forEach(([pid, delta]) => {
          ...
          supabase
            .from("players")
            .update({ stats: playerToStats({ ...p, ...delta }) })
            .eq("id", pid);
        });
      }
    },
  ...
  const confirmRemovePlayer = useCallback(
    (target) => {
      ...
      setFeed((prev) =>
        prev.filter(
          (m) =>
            !m.winnerIds?.includes(target.id) &&
            !m.loserIds?.includes(target.id)
        )
      );
      setShowRemovePlayer(false);
      if (leagueId) {
        supabase.from("players").delete().eq("id", target.id);
      }
    },
    [leagueId]
  );
            ...
            <div className="flex items-center justify-between gap-3">
                ...
                <div className="min-w-0">
                  <div
                    style={{
                      ...
                      letterSpacing: "3px",
                      color: N,
                      textShadow: "0 0 20px rgba(170,255,0,.4)",
                      lineHeight: 1,
                    }}
                  >
                    LEAGUE-IT
                  ...
                  <div
                    style={{
                      ...
                      fontWeight: 800,
                      marginTop: 2,
                      textShadow: "0 0 10px rgba(193,255,0,.4)",
                      overflow: "hidden",
                      textOverflow: "ellipsis",
                      whiteSpace: "nowrap",
                    }}
              ...
              {user?.user_metadata?.avatar_url ? (
                <img
                  ...
                  style={{ border: `2px solid ${N}44` }}
                />
              ) : (
                <div
                  className="flex items-center justify-center rounded-full w-9 h-9 text-[11px] font-black flex-shrink-0"
                  style={{
                    background: `linear-gradient(135deg,${N},#7DC900)`,
                    color: "#000",
                    fontFamily: "'DM Sans',sans-serif",
                ...
                >
                  {(user?.user_metadata?.full_name || user?.email || "YO")
                    .trim()
                    .split(/\s+/)
                    .map((w) => w[0].toUpperCase())
                    .slice(0, 2)
            ...
            >
              <motion.button
                ...
                style={{
                  ...
                  color: "#000",
                  border: "none",
                  boxShadow: "0 8px 28px rgba(170,255,0,.36)",
                }}
              >
                <div
                  className="absolute inset-0 pointer-events-none"
                ...
                />
                <span style={{ position: "relative" }}>Log Match</span>
              </motion.button>
            </div>
          )}
      ...
      <AnimatePresence>
        ...
        {showAddPlayer && (
          <motion.div
            ...
            onClick={(e) => {
              if (e.target === e.currentTarget) setShowAddPlayer(false);
            }}
          >
            <motion.div
            ...
            >
              ...
              >
                <div>
                  ...
                  >
                    They'll appear on the leaderboard immediately.
                  </p>
                </div>
                <button
              ...
              <div className="px-5 pt-5 pb-8">
                <input
                  ...
                  style={{
                    ...
                    background: "rgba(255,255,255,.05)",
                    border: `1.5px solid ${
                      addPlayerName.trim() ? N : "rgba(255,255,255,.12)"
                    }`,
                    color: "#fff",
                    caretColor: N,
                    boxShadow: addPlayerName.trim()
                      ? "0 0 18px rgba(170,255,0,.1)"
                      : "none",
                  }}
                />
                <PBtn
                ...
                >
                  {addPlayerName.trim()
                    ? `Add "${addPlayerName.trim()}" to League`
                    : "Enter a Name First"}
                </PBtn>
              </div>
            </motion.div>
        ...
        {showRemovePlayer && (
          <motion.div
            ...
            onClick={(e) => {
              if (e.target === e.currentTarget) setShowRemovePlayer(false);
            }}
          >
            <motion.div
              initial={{ y: "100%", opacity: 0 }}
            ...
            >
              ...
              >
                <div>
                  ...
                  >
                    All their matches will also be removed from the feed.
                  </p>
                </div>
                <button
                  onClick={() => setShowRemovePlayer(false)}
                  className="flex items-center justify-center w-8 h-8 rounded-full"
                  style={{
                    background: "rgba(255,255,255,.07)",
                    border: "1px solid rgba(255,255,255,.1)",
                  ...
                  .map((p) => (
                    <button
                      ...
                      style={{
                        ...
                        border: "1.5px solid rgba(255,51,85,.18)",
                        cursor: "pointer",
                      }}
                    >
                      <div
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        >
                          {p.wins}W · {p.losses}L · {p.totalPlayed} played
                        </div>
                      </div>
                      <UserMinus
                        size={16}
                        style={{ color: "#FF3355", flexShrink: 0 }}
                      />
                    </button>
                  ))}
              </div>
...
const SPORTS = [
  { id: "padel", label: "Padel", emoji: "🏸", sub: "Racket · Sets · Doubles" },
  {
    ...
    label: "Footvolley",
    emoji: "🏖️",
    sub: "Beach · No hands · Fun",
  },
  {
    id: "volleyball",
    label: "Volleyball",
    emoji: "🏐",
    sub: "Team · Rally scoring",
  },
  {
    id: "tennis",
    label: "Tennis",
    emoji: "🎾",
    sub: "Classic · Singles/Doubles",
  },
  {
    id: "pingpong",
    label: "Ping Pong",
    emoji: "🏓",
    sub: "Fast · Spin · 11 pts",
  },
  {
    id: "fifa",
    label: "FIFA",
    emoji: "🎮",
    sub: "Console · 6 min halves · 1v1",
  },
  {
    id: "custom_sport",
    label: "Other",
    emoji: "🏗️",
    sub: "Define your own game",
  },
];
...
const FORMATS = [
  {
    ...
    label: "Single Set",
    Icon: Zap,
    desc: "1 set decides everything — fastest format",
  },
  {
    id: "best3",
    label: "Best of 3",
    Icon: Target,
    desc: "First to win 2 sets takes the match",
  },
  {
    id: "points",
    label: "Points-Based",
    Icon: Hash,
    desc: "First to reach target points wins",
  },
  {
    id: "custom_logic",
    label: "Custom Rules",
    Icon: Settings,
    desc: "Describe your specific house rules",
  },
];
  ...
  return (
    <motion.button
      ...
      style={{
        ...
        boxShadow: disabled
          ? "none"
          : "0 8px 28px rgba(170,255,0,0.32), 0 2px 8px rgba(0,0,0,0.5)",
        fontSize: 15,
      }}
    >
      {!disabled && (
        ...
        {line2 && (
          <>
            ...
            <span
              style={{
                color: NEON,
                textShadow: `0 0 28px rgba(170,255,0,0.4)`,
              }}
            >
              {line2}
            </span>
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen px-6 text-center relative z-10">
      ...
      >
        ...
        <motion.div
          variants={fadeUp}
          className="flex items-center gap-3 my-5 w-full"
        >
          <div
            className="flex-1 h-px"
            style={{ background: "rgba(170,255,0,0.18)" }}
          />
          <span
            style={{
              fontSize: 9,
        ...
        <motion.div variants={fadeUp} className="w-full">
          <PrimaryBtn onClick={onSignIn || onNext}>Create My League</PrimaryBtn>
        </motion.div>
      </motion.div>
    </div>
  );
...
function StepSport({
  ...
  const canContinue =
    !!sport && (!isCustom || customSportName.trim().length > 0);
                ...
                <div className="relative">
                  <input
                    ...
                    style={{
                      ...
                      border: `1.5px solid ${
                        customSportName.trim() ? NEON : "rgba(170,255,0,0.38)"
                      }`,
                      color: "#fff",
                      caretColor: NEON,
                      fontFamily: "'DM Sans', sans-serif",
                ...
                >
                  ...
                  <div className="flex items-center gap-4 px-5 py-4">
                    ...
                    <Icon
                      ...
                      style={{
                        flexShrink: 0,
                        color: sel ? NEON : "rgba(255,255,255,0.28)",
                      }}
                    />
                  </div>
                  ...
                  <AnimatePresence>
                    {sel && (f.id === "points" || f.id === "custom_logic") && (
                      <motion.div
                        ...
                        style={{
                          overflow: "hidden",
                          borderTop: "1px solid rgba(170,255,0,0.14)",
                        }}
                      >
                        <div
                          className="px-5 py-4"
                        ...
                        >
                          {f.id === "points" && (
                            <>
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <button
                                  onClick={() =>
                                    setPoints(Math.min(50, points + 1))
                                  }
                                  className="flex items-center justify-center rounded-xl transition-all active:scale-90 flex-shrink-0"
                                  style={{
                                    width: 40,
                          ...
                          {f.id === "custom_logic" && (
                            <>
                              ...
                              >
                                {customRules.trim().length} chars · Rules will
                                be pinned to your league's info page.
                              </p>
                            </>
                          )}
                        </div>
            ...
            >
              ...
              <div
                ...
                style={{
                  height: 1.5,
                  background:
                    "linear-gradient(90deg, transparent, rgba(170,255,0,0.5), transparent)",
                }}
              />
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  {
                    label: "SPORT",
                    value: `${resolvedEmoji} ${resolvedSport}`,
                  },
        ...
        >
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="text-left min-w-0">
                ...
                >
                  {resolvedEmoji} {resolvedSport} · {formatLabel} · 👑
                  Commissioner
                </div>
              </div>
            </div>
          ...
          >
            <div
              ...
              style={{
                height: 1.5,
                background:
                  "linear-gradient(90deg, transparent, rgba(170,255,0,0.55), transparent)",
              }}
            />
            <p
              style={{
          ...
          <motion.div variants={fadeUp} className="w-full mb-2.5">
            <motion.button
              ...
              style={{
                ...
                color: "#fff",
                boxShadow:
                  "0 8px 30px rgba(37,211,102,0.38), 0 2px 8px rgba(0,0,0,0.45)",
              }}
            >
              <div
                className="absolute inset-0 pointer-events-none"
          ...
          >
            ...
            >
              ...
              <span style={{ color: NEON }}>{fullLink}</span>. No more excuses.
              Let's see who's #1. 😤
            </p>
          </motion.div>
  ...
  return (
    <div className="flex flex-col h-full">
          ...
          <motion.div variants={fadeUp} className="relative mb-2">
            <input
              ...
              style={{
                ...
                color: "#fff",
                caretColor: NEON,
                boxShadow: adminName.trim()
                  ? "0 0 24px rgba(170,255,0,0.1)"
                  : "none",
              }}
            />
          </motion.div>
      ...
      <FixedFooter>
        <PrimaryBtn onClick={onNext} disabled={!canSubmit}>
          {canSubmit
            ? `Let's go, ${adminName.trim().split(" ")[0]}! 🚀`
            : "Enter Your Name"}
        </PrimaryBtn>
      </FixedFooter>
    </div>
        ...
        >
          <div
            style={{
              ...
              letterSpacing: "6px",
              color: N,
              textShadow: "0 0 32px rgba(170,255,0,.5)",
              lineHeight: 1,
              marginBottom: 8,
            }}
          >
          ...
          <motion.button
            ...
            style={{
              ...
              cursor: "pointer",
              fontFamily: "'DM Sans',sans-serif",
              boxShadow: "0 8px 28px rgba(255,255,255,.14)",
          ...
          <div style={{ padding: "20px 20px 0", flex: 1, overflowY: "auto" }}>
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              <motion.button
                ...
                style={{
                  background: `linear-gradient(135deg,${N}18,${N}08)`,
            ...
            >
              ...
              <div className="px-5 pt-5 pb-8">
                ...
                <PBtn
                  ...
                  style={{
                    background: "linear-gradient(135deg,#3B8EFF,#1A5FCC)",
                  }}
                >
...
export default function Root() {
  ...
  useEffect(() => {
    console.log("[LeagueIt] Initializing auth...");
    ...
    const fallback = setTimeout(() => {
      setPhase((prev) => {
        if (prev === "loading") {
          console.log(
            "[LeagueIt] Auth timeout — no session detected, showing onboarding"
          );
          return "login";
        }
        return prev;
  ...
  const handleWizardFinish = useCallback(
    async ({
      ...
      const sportEmoji =
        sport === "custom_sport" ? "🏗️" : sportData?.emoji || "🏸";
      const { data: leagueRow, error } = await supabase
        .from("leagues")
        .insert({
          name,
          sport: sportLabel,
          settings: { leagueCode, format, points, customRules },
        })
        .select()
        .single();
      if (error || !leagueRow) {
